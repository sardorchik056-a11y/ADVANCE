...
def _create_session(difficulty: int, bet: float, chat_id: int, owner_id: int = 0) -> dict:
    ...
    for _ in range(FLOORS):
        floors.append({'bomb_cols': [], 'chosen': None, 'is_bomb': None})
    return {
...
def build_tower_keyboard(session: dict, game_over: bool = False) -> InlineKeyboardMarkup:
    ...
    for floor_idx in range(FLOORS - 1, -1, -1):
        ...
        btn_row.append(InlineKeyboardButton(text=f"x{mult}", callback_data="tower_noop"))
        ...
        if game_over:
            ...
            for col in range(CELLS):
                btn_row.append(InlineKeyboardButton(
                    text=CELL_ACTIVE, callback_data=f"tower_cell_{floor_idx}_{col}"
                ))
        else:
            for col in range(CELLS):
    ...
    if not game_over:
        ...
        if floors_passed > 0:
            ...
            ctrl.append(InlineKeyboardButton(
                text=f"Забрать {cashout}", callback_data="tower_cashout", icon_custom_emoji_id=EMOJI_GOAL
            ))
        ctrl.append(InlineKeyboardButton(text="Выйти", callback_data="tower_exit", icon_custom_emoji_id=EMOJI_BACK))
        rows.append(ctrl)
    else:
        rows.append([
            InlineKeyboardButton(text="Снова", callback_data="tower_play_again", icon_custom_emoji_id=EMOJI_3POINT),
            InlineKeyboardButton(text="Выйти", callback_data="tower_exit",       icon_custom_emoji_id=EMOJI_BACK),
        ])
...
async def show_tower_menu(callback: CallbackQuery, storage, betting_game=None):
    ...
    await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=build_tower_select_keyboard())
...
async def tower_diff_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="Назад", callback_data="tower_back_select", icon_custom_emoji_id=EMOJI_BACK)
        ]])
    )
...
async def tower_exit(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        get_games_menu_text(caller_id), parse_mode="HTML", reply_markup=get_games_menu()
    )
    await callback.answer()
...
async def tower_cell_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if is_bomb:
            ...
            record_game_result(user_id, name, bet, 0.0, GAME_NAME)
            ...
            if session['current_floor'] >= FLOORS:
                ...
                name = callback.from_user.first_name or callback.from_user.username or f"User {user_id}"
                record_game_result(user_id, name, bet, winnings, GAME_NAME)
                ...
                await callback.message.edit_text(
                    ...
                    parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="Играть снова", callback_data="tower_cashout_again", icon_custom_emoji_id=EMOJI_3POINT)],
                        [InlineKeyboardButton(text="Выйти",        callback_data="tower_cashout_exit",  icon_custom_emoji_id=EMOJI_BACK)],
                    ])
                )
                set_owner_fn(callback.message.message_id, user_id)
                ...
                await callback.message.edit_text(
                    game_text(session), parse_mode=ParseMode.HTML, reply_markup=build_tower_keyboard(session)
                )
...
async def process_tower_command(message: Message, state: FSMContext, storage):
    ...
    match = re.match(r'^(?:/)?(?:tower|башня)\s+([\d.,]+)\s+(\d+)$', text, re.IGNORECASE)
    ...
    if difficulty not in (1, 2, 3, 4):
        await message.answer("<blockquote><b>❌ Сложность должна быть 1, 2, 3 или 4.</b></blockquote>", parse_mode=ParseMode.HTML)
        return
    if bet < 0.1:
        await message.answer("<blockquote><b>❌ Минимальная ставка: 0.1</b></blockquote>", parse_mode=ParseMode.HTML)
        return
    if bet > 10000:
        await message.answer("<blockquote><b>❌ Максимальная ставка: 10 000</b></blockquote>", parse_mode=ParseMode.HTML)
        return
