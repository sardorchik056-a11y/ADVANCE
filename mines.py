...
def build_game_keyboard(session: dict, game_over: bool = False) -> InlineKeyboardMarkup:
    ...
    if not game_over:
        ...
        rows.append([
            InlineKeyboardButton(text="Снова", callback_data="mines_play_again", icon_custom_emoji_id=EMOJI_3POINT),
            InlineKeyboardButton(text="Выйти", callback_data="mines_exit",       icon_custom_emoji_id=EMOJI_BACK),
        ])
...
def build_mines_select_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        row,
        [InlineKeyboardButton(text="Ввести вручную", callback_data="mines_manual", icon_custom_emoji_id=EMOJI_NUMBER)],
...
async def show_mines_menu(callback: CallbackQuery, storage, betting_game):
    ...
    await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=build_mines_select_keyboard())
...
async def mines_cashout(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Играть снова", callback_data="mines_cashout_again", icon_custom_emoji_id=EMOJI_3POINT)],
            [InlineKeyboardButton(text="Выйти",        callback_data="mines_cashout_exit",  icon_custom_emoji_id=EMOJI_BACK)],
        ])
    )
    set_owner_fn(callback.message.message_id, user_id)
...
async def process_mines_bet(message: Message, state: FSMContext, storage):
    ...
    sent = await message.answer(
        game_text(session), parse_mode=ParseMode.HTML, reply_markup=build_game_keyboard(session)
    )
...
async def process_mines_command(message: Message, state: FSMContext, storage):
    ...
    match = re.match(r'^(?:/)?(?:mines|мины)\s+([\d.,]+)\s+(\d+)$', text, re.IGNORECASE)
    ...
    if mines_count < 2 or mines_count > 24:
        await message.answer("<blockquote><b>❌ Количество мин должно быть от 2 до 24.</b></blockquote>", parse_mode=ParseMode.HTML)
        return
    if bet < 0.1:
