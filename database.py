...
def db_get_user_last_games(user_id: int, limit: int = 10) -> list:
    """Возвращает последние `limit` игровых результатов пользователя."""
    try:
        with _connect() as conn:
            cur = conn.execute("""
                SELECT game_name, win_amount, created_at
                FROM game_results
                WHERE user_id = ?
                ORDER BY created_at DESC
                LIMIT ?
        ...
        logging.error(f"[DB] db_get_user_last_games user_id={user_id}: {e}")
        return []
