...
REQUIRED_NAME_SUBSTRING = "@egodicebot"
...
async def _handle_bonus_locked(message: Message, user_id: int) -> None:
    ...
    if not name_ok:
        ...
        await message.answer(
            ...
            "В <b>никнейме</b> должна быть: <code>@egodicebot</code>\n"
            "  Пример: <code>Иван @egodicebot</code>\n\n"
            "<tg-emoji emoji-id=\"" + EMOJI_LOSS + "\">❌</tg-emoji> "
            "В <b>«О себе»</b> должна быть:\n"
            "  <code>честная игровая зона-@egodicebot</code>\n\n"
                    ...
                    try:
                        await _bot.send_message(
                            ...
                            text=(
                                ...
                                "<blockquote>"
                                "Вы убрали <code>@SCTGame_Bot</code> из никнейма.\n\n"
                                "<tg-emoji emoji-id=\"" + EMOJI_CLOCK + "\">⏰</tg-emoji> "
