...
try:
    ...
    def record_game_result(user_id, name, bet, win, game_name=""): pass
...
def _create_session(bet: float, chat_id: int, owner_id: int = 0) -> dict:
    ...
    for _ in range(FLOORS):
        floors.append({'bomb_col': None, 'chosen': None, 'is_bomb': None})
    return {
        'bet':              bet,
        'current_floor':    0,
        'floors_passed':    0,
...
def build_gold_keyboard(session: dict, game_over: bool = False) -> InlineKeyboardMarkup:
    ...
    for floor_idx in range(FLOORS - 1, -1, -1):
        ...
        btn_row.append(InlineKeyboardButton(text=f"x{mult}", callback_data="gold_noop"))
        ...
        if game_over:
            ...
            for col in range(CELLS):
                btn_row.append(InlineKeyboardButton(
                    text=CELL_HIDDEN, callback_data=f"gold_cell_{floor_idx}_{col}"
                ))
        else:
            for col in range(CELLS):
    ...
    if not game_over:
        ...
        if floors_passed > 0:
            ...
            ctrl.append(InlineKeyboardButton(
                text=f"Забрать {cashout}", callback_data="gold_cashout", icon_custom_emoji_id=EMOJI_GOAL
            ))
        ctrl.append(InlineKeyboardButton(text="Выйти", callback_data="gold_exit", icon_custom_emoji_id=EMOJI_BACK))
        rows.append(ctrl)
    else:
        rows.append([
            InlineKeyboardButton(text="Снова", callback_data="gold_play_again", icon_custom_emoji_id=EMOJI_3POINT),
            InlineKeyboardButton(text="Выйти", callback_data="gold_exit",       icon_custom_emoji_id=EMOJI_BACK),
        ])
...
async def gold_cell_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if is_bomb:
            ...
            record_game_result(user_id, display_name, bet, 0.0, GAME_NAME)
            ...
            if session['current_floor'] >= FLOORS:
                ...
                record_game_result(user_id, display_name, bet, winnings, GAME_NAME)
                ...
                await callback.message.edit_text(
                    ...
                    parse_mode=ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="Играть снова", callback_data="gold_cashout_again", icon_custom_emoji_id=EMOJI_3POINT)],
                        [InlineKeyboardButton(text="Выйти",        callback_data="gold_cashout_exit",  icon_custom_emoji_id=EMOJI_BACK)],
                    ])
                )
                set_owner_fn(callback.message.message_id, user_id)
                _game_board_owner[callback.message.message_id] = user_id
                ...
                await callback.message.edit_text(
                    game_text(session), parse_mode=ParseMode.HTML, reply_markup=build_gold_keyboard(session)
                )
...
async def _start_game(send_fn, user_id: int, bet: float, chat_id: int,
                      bot: Bot, storage, state: FSMContext, from_user):
    ...
    sent = await send_fn(game_text(session), build_gold_keyboard(session))
...
async def process_gold_bet(message: Message, state: FSMContext, storage):
    ...
    async with bet_lock:
        ...
        await _start_game(
            send_fn=_send, user_id=user_id, bet=bet,
            chat_id=message.chat.id, bot=message.bot,
            storage=storage, state=state, from_user=message.from_user,
        )
